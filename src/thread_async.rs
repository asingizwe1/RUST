...
//we can also make our hamburger concurrently using threads
//too many threads can crash program
let mut handles = vec![];
for i in 0..10 {
    let handle = std::thread::spawn(move || {
        println!("Making hamburger in thread {}", i);
        //simulate some work
        std::thread::sleep(std::time::Duration::from_millis(500));
        println!("Hamburger {} is ready!", i);
    });
    handles.push(handle);
