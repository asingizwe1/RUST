...
// this method alters the Measurement struct
pub fn visit_doctor(&mut self, measurements: Measurements)-> HealthReport {
   //method takes up Measuremnt struct as input and returns Healthreport struct summarising changes
   //so thats why we have both structs noted
    ...
    //for this case we are chaning properties of struct Measurements 
    self.visit_count += 1;
    // method extracts the current blood pressure (bp) from the measurements struct.
    let bp = measurements.blood_pressure;
    //. Create the HealthReport
    let report = HealthReport {
    patient_name: &self.name,
    visit_count: self.visit_count as u32,
    height_change: measurements.height- self.height,
    blood_pressure_change: match self.last_blood_pressure {
    //match self.last_blood_pressure in this context is determined by what the match expression evaluates to
    //match statement evaluates self.last_blood_pressure and branches into different logic depending on its value:
    Some(lbp) => {
    ...
    //you must indicate what is  retutned at the end
    //in this case its report which is what we declares as our health report
    report
    }
    }
    fn main() {
