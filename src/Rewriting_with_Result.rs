...
//peek() is a method in Rust’s Peekable iterator that
// allows you to look at the next character without moving forward.
use std::str::Chars;
...
}//The ? operator is used to propagate errors that return Result<T,E>
// It unwraps the Ok value if the operation is successful and returns the error if it’s Err.
fn main() -> anyhow::Result<()> {
let expr = parse("10+foo+20-30")?;
println!("{expr:?}");
//returns anyhow::Result<()>, which:Automatically converts ParserError into an error that can be printed.
Ok(())
}
//parse() is a Rust method that converts a string into a number (or another type). 
//It’s often used with ? to handle errors.
