...
fn count(&mut self, value:T){
    *self.values.entry(value).or_default() += 1;//copied() or * - derefrencing
...
fn times_seen(&self, value: T)-> u64 {
    ...
    }//copied()- When you convert a &u64 (a reference to a 64-bit unsigned integer) into a u64 (the actual value), you are dereferencing the reference to obtain the underlying value
}
